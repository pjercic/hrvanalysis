...
def get_jamzone_time_domain_features(nn_intervals: List[float], timestamp_list: List[str], window_duration: str) -> dict:
    """
    Returns a dictionary containing time domain features for HRV analysis.
    Mostly used on long term recordings (24h) but some studies use some of those features on
    short term recordings, from 1 to 5 minutes window.
    ...
    rest_score = np.minimum(avg_rmssd / 10, 10)
    balance_score =  (np.abs(focus_range_ratio_rmssd - calm_range_ratio_rmssd) - 1) * (-10)
...
def get_jamzone_frequency_domain_features(nn_intervals: List[float], method: str = LOMB_METHOD,
    ...
    frequency_domain_features['balance_group'] = np.maximum(np.log2(frequency_domain_features['lf_hf_ratio'] * 100 / 25), 0.0) + 1
    frequency_domain_features['balance_score'] = np.maximum((- np.square(frequency_domain_features['balance_group']) * 9 / 4) + (27 / 2 * frequency_domain_features['balance_group']) - (41 / 4), 1.0)
